...
def main():
    ...
    """
    input_string0 = "8123456789abcdeffedcba9876543210"
    input_string1 = "8023456789abcdeffedcba9876543210"
    input_string2 = "a123456789abcdeffedcba9876543210"
    input_key = "0f1571c947d9e8591cb7add6af7f6798"

    print("\nKey:\n", input_key, "\n")

    # Initial State
    print("Initial State &", input_string0, "&", bit_diff(input_string0, input_string1), "\\\\")
    print("&", input_string1, "&", "\\\\")
    print("\\hline")
   ...
   # Round 0
    state0 = bytes_to_state(input_string0)
    state1 = bytes_to_state(input_string1)

    round_key = key_to_matrix(input_key)
    round_key_state = matrix_to_state(round_key)
        
    state0 = add_round_key(state0, round_key_state)
    state1 = add_round_key(state1, round_key_state)

    round_result0 = state_to_bytes(state0)
    round_result1 = state_to_bytes(state1)

    print("0 &", round_result0, "&", bit_diff(round_result0, round_result1), "\\\\")
    print("&", round_result1, "&", "\\\\")
    print("\\hline")
    ...
    for round_ in range(9):
        substituted0 = substitute_bytes(state0, AES_SBOX)
        substituted1 = substitute_bytes(state1, AES_SBOX)

        shifted0 = shift_rows(substituted0)
        shifted1 = shift_rows(substituted1)

        mixed0 = mix_columns(shifted0, AES_MIX_COL)
        mixed1 = mix_columns(shifted1, AES_MIX_COL)

        round_key = iterate_round_key(round_key, Rcons[round_])
        round_key_state = matrix_to_state(round_key)

        state0 = add_round_key(mixed0, round_key_state)
        state1 = add_round_key(mixed1, round_key_state)

        round_result0 = state_to_bytes(state0)
        round_result1 = state_to_bytes(state1)

        print(round_ + 1, "&", round_result0, "&", bit_diff(round_result0, round_result1), "\\\\")
        print("&", round_result1, "&", "\\\\")
        print("\\hline")
    ...
    state0 = add_round_key(shifted0, round_key_state)
    state1 = add_round_key(shifted1, round_key_state)

    round_result0 = state_to_bytes(state0)
    round_result1 = state_to_bytes(state1)

    print("10 &", round_result0, "&", bit_diff(round_result0, round_result1), "\\\\")
    print("&", round_result1, "&", "\\\\")
    print("\\hline")
...
def state_to_bytes(state_array):
    output = ""

    for row in range(4): 
         for col in range(4):
             output += state_array[col][row]
...
def bit_diff(hex_string1, hex_string2):
    different = "{:b}".format(int(hex_string1, 16) ^ int(hex_string2, 16))
    return(different.count("1"))
